...
def prepare_timit_batch(features, labels, lengths, repeat_steps=5):
    if repeat_steps > 1:
        features = features.repeat_interleave(repeat_steps, dim=1)
        labels = labels.repeat_interleave(repeat_steps, dim=1)
        lengths = lengths * repeat_steps

    B, T, _ = features.shape

    time_idx = torch.arange(T, device=features.device).unsqueeze(0)   # [1, T]
    valid = (time_idx < lengths.unsqueeze(1)).float()                 # [B, T]
    weighted_mask = valid / lengths.unsqueeze(1).float()              # [B, T]

    x_seq = features.transpose(0, 1).contiguous()                     # [T, B, n_in]
    targets = labels.transpose(0, 1).contiguous()                     # [T, B]
    weighted_mask = weighted_mask.transpose(0, 1).contiguous()        # [T, B]

    return x_seq, targets, weighted_mask
...
def train_one_epoch(
    ...
    for batch_idx, (features, labels, lengths) in enumerate(train_loader):
        ...
        with torch.no_grad():
            m = masked_metrics(y_out, targets, weighted_mask, z_rec)
